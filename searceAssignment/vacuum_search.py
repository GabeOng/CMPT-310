...
"""
costFunctions = ['Step', 'StepTurn', 'StayLeft', 'StayTop']
...
class VacuumPlanning(Problem):
    ...
    def path_cost(self, curNode, state1, action, state2):
        ...
        return self.computeTurnCost(curNode.action, action)+curNode.path_cost
    ...
    def computeTurnCost(self, action1, action):
        ...
        if(env.costFunc=='StayLeft'):
            return stayLeft.get((action1, action), 0)
        if(env.costFunc=='StayTop'):
            #print('stayingup', action1, action, stayUp.get((action1, action), 1))
            return stayUp.get((action1, action), 0.5)
        if(env.costFunc=='StepTurn'):
            return turns.get((action1, action), 0)
        return 1
    ...
    def findMinManhattanDist(self, pos):
        ...
        for dirtyRoom in self.env.dirtyRooms:
            distance = distance_manhattan(pos, dirtyRoom)
            if distance<minDist:
                minDist = distance
        return minDist
    ...
    def findMinEuclidDist(self, pos):
        ...
        print(" Your code goes here\n")
        for dirtyRoom in self.env.dirtyRooms:
            distance = distance_squared(pos, dirtyRoom)
            if distance<minDist:
                minDist = distance
        return minDist
    ...
    def h(self, node):
        ...
        r=self.findMinManhattanDist(node.state)
        print(r)
        return r
