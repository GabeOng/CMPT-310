...
def breadth_first_graph_search(problem):
    ...
    while frontier:
        cur=frontier.pop(0)
        reached.add(tuple(cur.state))
        for action in problem.actions(cur.state):
            child=cur.child_node(problem,action)
            s=child.state
            if problem.goal_test(s):
                
                print(frontier)
                return child, reached
            if tuple(s) not in reached and child not in frontier:
...
def best_first_graph_search(problem, f=None):
    ...
    while frontier:
        ...
        #if problem goal then return solution
        if problem.goal_test(cur.state):
            return cur, explored
        
        #add node to explored
        explored.add(tuple(cur.state))
        
        #for each action in problem find child
        for action in problem.actions(cur.state):
            child = cur.child_node(problem, action)
            #if child not in explored or in frontier add child to frontier
            #if greedy and tuple(child.state) not in explored and child not in frontier:
                #frontier.append(child)
            if g:
                if tuple(child.state) not in explored and child not in frontier:
                    frontier.append(child)
                #elif child in frontier with a higher path cost replace
                elif child in frontier and child.path_cost < frontier[child]:
                    print('deleted')
                    del frontier[child] 
                    frontier.append(child)
            else:
                if tuple(child.state) not in explored or child not in frontier:
                    frontier.append(child)
                #elif child in frontier with a higher path cost replace
                elif child in frontier and child.path_cost < frontier[child]:
                    print('deleted')
